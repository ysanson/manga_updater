...
pub fn restore_file(restore_path: Option<PathBuf>) -> Result<(), io::Error> {
    let path = extract_path_or_default(&restore_path);
    if !path.is_file() {
...
mod tests {
    ...
    use crate::file_ops::{create_file, update_csv};
