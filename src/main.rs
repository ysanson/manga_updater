...
use crate::commands::{list, init, add, update, export, import, remove, open, unread, restore};
...
struct Cli {
    ...
    help="Available commands: list, add [url], remove [url], export [-e path], import [-e path], update [url/all], open [url/line number], restore [-e path]. For more info, refer to the doc.")]
    command: String,
...
async fn main() {
    ...
    match args.command.as_str() {
        ...
        "restore" => restore(args.path, args.verbose),
